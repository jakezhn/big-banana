...
原因：
文件中 `timeframe_label` 已明确是给人和 LLM 读的，并且示例里已有 `5m / 15m / 1h / 4h / 1d` 这类标签；所以 `5m -> 15m -> 1h -> 4h` 的上下级可以自然建立。对于 1d，如果不引入 1w，你会缺“高一档”结构锚。
...
我建议把**主计划**锚定在 15m / 1h / 4h / 1d，但“辅助周期”允许延伸。
